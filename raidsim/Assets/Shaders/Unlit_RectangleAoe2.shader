...
Shader "Custom/Unlit/RectangleAoe2"
{
    Properties
    {
        [HideInInspector]
    ...
    CustomEditor "dev.susybaka.raidsim.Editor.RectangleAoeShader2Inspector"
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutput o)
        {
            ...
            // Use the built-in _Time.y to create a sawtooth wave for the sweeping line
            float time = _Time.y * _PulseSpeed;
            float cycleTime = frac(time);

            // Calculate the position of the sweeping line
            float linePosition = cycleTime * (_OuterWidth * 2.0) - _OuterWidth; // Sweeps from left to right

            // Calculate the fading effect for the sweeping line
            ...
            // Sweeping line opacity
            float lineOpacity = 0.0f;
            if (abs(uv.x - linePosition) < _LineSize && distY < _OuterHeight && distX < _OuterWidth && angle <= angleToRad(_Angle))
            {
                // Ensure the line is only visible within the outer rectangle and outside the inner rectangle
                if (distX > innerWidth || distY > innerHeight)
                {
                    lineOpacity = lerp(0.0f, _MaxFill, 1.0 - abs(uv.x - linePosition) / _LineSize) * fadeEffect * _InnerOpacity;
                }
            }
            ...
            float combinedOpacity = saturate(outerOpacity + lineOpacity + glowOpacity + outlineOpacity);

            // Apply tints based on the contribution of inner, outer, glow, and outline
            fixed3 finalColor = 0;
            if (lineOpacity > 0)
            {
                finalColor = _InnerTintColor.rgb * lineOpacity;
            }
            if (glowOpacity > 0)
            {
                finalColor += _GlowTintColor.rgb * glowOpacity * (1 - lineOpacity);
            }
            if (outerOpacity > 0)
            {
                finalColor += _TintColor.rgb * outerOpacity  * (1 - glowOpacity) * (1 - lineOpacity);
            }
            if (outlineOpacity > 0)
            {
            ...
            float innerAlpha = lineOpacity > 0 ? _InnerTintColor.a * lineOpacity : 0.0;
