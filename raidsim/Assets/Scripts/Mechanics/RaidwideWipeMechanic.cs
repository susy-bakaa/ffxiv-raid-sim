...
public class RaidwideWipeMechanic : FightMechanic
{
    ...
    public override void TriggerMechanic(ActionInfo actionInfo)
    {
        ...
        if (party != null)
        {
            Utilities.FunctionTimer.Create(() => FightTimeline.Instance.WipeParty(party, mechanicName), 0.1f, $"TriggerMechanic_{this}_{GetHashCode()}_PartyWipe_Delay", false, true);
        }
        else
        {
            Utilities.FunctionTimer.Create(() => FightTimeline.Instance.WipeParty(FightTimeline.Instance.partyList, mechanicName), 0.1f, $"TriggerMechanic_{this}_{GetHashCode()}_PartyWipe_Delay", false, true);
        }
    }
}